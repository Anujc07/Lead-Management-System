...
def Dashboard(request):
    ...
    if teamIDs and start_date and end_date:    
        ...
        # home_visit = FollowUpData.objects.filter(EnquiryStage='Home Visit - Done').count()
    ...
    return render(request, 'Admin/dashboard.html', {'bookings': bookings, 'new_leads': new_leads,
                                                     ...
                                                     'home_visit': home_visit, 'corporate_visit': corporate_visit, 'start_date':start_date, 'end_date':end_date})
    ...
    if start_date and end_date:
        ...
        for team in teams:
            ...
            for member in members:
                ...
                # home_solo = HomeVisit.objects.filter(Q(name=member) & Q(visit_type = 'solo')).count()
                # home = HomeVisit.objects.filter(Q(name=member) & Q(visit_type = 'team') & Q(date__range=(start_date, end_date))).values_list('co_fellow', flat=True)
                ...
                #         home_count = HomeVisit.objects.filter(Q(name=co_fellow_name) & Q(co_fellow__contains=member) | Q(co_fellow__startswith=member + ',') |
                #                 Q(co_fellow__endswith=',' + member) | Q(co_fellow__icontains=',' + member + ',')).count()                        
                #         total_home_count += home_count                    
                # # print(total_home_count+home_solo)
                # Home_visit = total_home_count+home_solo
...
def DPR_Date_Range(emp, start_date, end_date, request):
    ...
    for employee, employee_id  in emp:       
        # employee = empl[0]
        # employee_id = empl[1]
        # home_solo = HomeVisit.objects.filter(Q(name=employee) & Q(visit_type = 'solo')).count()
        # home = HomeVisit.objects.filter(Q(name=employee) & Q(visit_type = 'team')).values_list('co_fellow', flat=True)
        # fellow_data = list(home)
        # co_fellow_data = list(set(fellow_data))
        # total_home_count = 0
        ...
        #     for co_fellow_name in co_fellow_list:
        #         # print('============= name of co-fellow:', co_fellow_name)
        #         home_count = HomeVisit.objects.filter(Q(name=co_fellow_name) & Q(co_fellow__contains=employee) | Q(co_fellow__startswith=employee + ',') |
        #                 Q(co_fellow__endswith=',' + employee) | Q(co_fellow__icontains=',' + employee + ',')).count()                        
        #         total_home_count += home_count                    
        # # print(total_home_count+home_solo)
        # home_visit = total_home_count+home_solo
        ...
        lead_FW = FollowUpData.objects.filter(Employee_Name=employee, FollowUp_Date__range=(start_date, end_date_present)).count()
